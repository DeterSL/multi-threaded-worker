...
namespace detersl::runner {
    ...
    Runner::Runner(acquired_cown_span<detersl::types::Resource> rw_cown_arr,
        ...
        size_t ro_index = 0;
        size_t rw_index = 0;
        for (const auto& res_name : basic_func.resources) {
            if(basic_func.read_only_resources.find(res_name) != basic_func.read_only_resources.end()){
        ...
        storage = std::make_unique<detersl::kv::ResourceStorage>(std::move(local_resources),  
            basic_func.read_only_resources);       
    }
