...
void register_and_schedule_cpp(){
    httplib::Server server;
    server.new_task_queue = [] { return new httplib::ThreadPool(1); };

    server.Post("/func", [&](const httplib::Request& req, httplib::Response& res) {
        if (req.body.empty()) {
            res.status = 400;
        ...
        std::string func_name = req.body;
       
        std::string error;
   
        int func_id = 0;
        if (detersl::cpp_worker::register_function(func_name, &error, &func_id) != 0) {
            res.status = 400;
    ...
    server.Post("/workflow/register", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        if (detersl::cpp_worker::register_workflow(workflow, &error) != 0) {
            res.status = 400;
    ...
    server.Post("/workflow/invoke", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        try {
         ...
         if (wf_count >= delete_after_n_wf) {
            wf_count = 0;
            detersl::cpp_worker::cleanup_resources();
        }

        if (!detersl::cpp_worker::invoke_workflow(invoke, &error)) {
            res.status = 400;
    ...
    server.Get("/resource/:res_name", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        bool found = detersl::cpp_worker::get_resource(res_name, res_data);
    ...
    std::cout << "Listening for CPP configs on http://0.0.0.0:6665\n";
    server.listen("0.0.0.0", 6665);
}
